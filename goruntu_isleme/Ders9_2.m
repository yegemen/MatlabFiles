...
se = ones(1,71); 
fo = imopen(f,se);  % Görüntünün arka planı elde edildi.
subplot(131), imshow(f)
...
title({'Açmadan sonra' 'kapanmış görüntü'})
subplot(335), imshow(foci);
title({'Kapamadan sonra' 'açılmış görüntü'})
subplot(336), imshow(fasf);
title({'Açmadan sonra kapanmış' 'çeşitli yarıçaplarda görüntü'})
% Enlarge figure to full screen.
set(gcf, 'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
