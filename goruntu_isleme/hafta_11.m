...
%% Ba?lant?l? bile?enlerin ç?kar?lmas?
close all;clc;
bw = imread('basic_shapes.png');   %Read in binary(or logical) image
%baglantili bilesenleri belirlemek için bwlabel i siyah beyaz görüntüye
%uygulayabiliyoruz.
...
c1.Label.String = 'Görüntü renk skalas?';
subplot(122), imagesc(L);  %Display labelled image , etiketli görüntüyü ekrana bast?r?yoruz.
title('Ba?lant?l? Bile?enli Görüntü')
...
%% Ba?lant?l? bile?elerin kütle merkezini hesaplama ve gösterimi.
...
%% Ba?lant?l? bile?enlerin kümesini tan?mlama
clear;close all;clc
...
title({'?kili görüntünün' 'ba?lant?l? bile?enleri'})

se = strel('disk',10);
f2 = imdilate(f,se);
subplot(323), imshow(f2)
title({'Geni?letilmi? ikili görüntünün'})

[L2,num2] = bwlabel(f2);
g2 = label2rgb(L2,'jet',[.7 .7 .7],'shuffle');
subplot(324), imshow(g2)
title({'Geni?letilmi? ikili görüntünün' 'ba?lant?l? bile?enleri'})

L3 = L2;
L3(~f) = 0; % f görüntüsündeki tüm arka plan konumlar?na 0 ata.
g3 = label2rgb(L3,'jet', [.7 .7 .7],'shuffle');
subplot(325), imshow(g3)
title({'Kümelemeye göre renklendirilmi?' 'nesnelerle orijinal görüntü'})
...
%% Vur-yada-iskala dönüsümü: noisy image, onceki goruntunun gurultulu hali
...
%% Vur-yada-iskala dönüsümü--Umrumda degil
...
title('Gürültü ve kö?e noktalar') 
subplot(325), imshow(A-(bw2-bw1))  %Gürültü kald?r?lm?? görüntü
...
%% Örnek-1 Çesitli geometrik sekillerin iskeletleri
clear;close all;clc;
...
title('?skelet') 
%?skeletdeki ç?k?nt?lar komutu kullanarak azalt?l?r.
% Enlarge figure to full screen.
set(gcf, 'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);

%% Örnek-2 Gürültülü görüntü--iskelet çikarma
clear;close all;clc;
...
%Sonra, düzgünlestirilmis görüntüyü esiklileme:

g = imbinarize(g, 1.7*graythresh(g));
subplot(333), imshow(g) % Fig. (c)
title('Threshold image') 
%?skeleti ç?karan komut:
...
title('Ç?k?nt?lar? kald?r?lm?? iskelet') 
% Enlarge figure to full screen.
set(gcf, 'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
figure, imshow(s1)
...
title('Açma disk yar?çap? = 10')
se = strel('square',25); 
Abw1 = imerode(A,se);
imshow(A-Abw1)
bw = imopen(A,se);             %Open with square side 25
subplot(1,3,3), imshow(bw)      
title('Açma kare kenar? = 25')

%% Morfolojik açma-3: ?stenmeyen gürültüyü kald?rma
...
%% Morfolojik rekonstrüksiyonun kullan?m?
mask = ~imread('enhance_text.png'); %Read in binary text
figure, 
subplot(221), imshow(mask);
title('mask görüntü')
%mask = imclose(A,ones(3)); %Harflerdeki köprü k?r?lmalar?na kapat?r
...
title({'Açmayla yeniden' 'yap?land?r?lm?? görüntü'})
% Enlarge figure to full screen.
set(gcf,'Units','Normalized','OuterPosition',[0 0 1 1]);
%saveas(gcf,[filepath 'opening_text.png'])  %save figure

%% morfolojik rekonstrüksiyonun kullan?m?-2
mask = ~imread('enhance_text.png');  %Read in binary text
...
title({'Dikey çizgiyle' 'Morfolojik açarak yap?land?rma'})
subplot(334),imshow(~fobr);
title({'Yeniden yap?land?rmayla' ' aç?lm?? görüntü'})
subplot(335),imshow(~g);title('Doldurulmu? orijinal görüntü')
subplot(336),imshow(~gborder);
title({'S?n?r karakterleri' 'kald?rm?? görüntü'})
%saveas(gcf,[filepath 'opening_text.png'])  %save figure

%% Yeniden yap?land?rmayla açma-reconstruction
...
title('Geni?letilmi? görüntü')
ge = imerode(f, se);
subplot(223), imshow(ge)
title('A??nd?r?lm?? görüntü')
morph_grad = gd - ge;
...
title({'morfolojik açmayla' 'ayd?nlatman?n tahmini'})
subplot(233), imshow(I2,[]);
title({'orijinalden ayd?nlatman?n' 'ç?kar?lmas?n?n sonucu'})
subplot(234), imshow(g,[]);
title('Orijinal + imtophat - imbothat')
% Enlarge figure to full screen.
set(gcf, 'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);

%% Morfolojik açma-2: Remove a complex background. 
f = imread('Fig0930(a).tif');     %Read in binary text
se = ones(1,71); 
fo = imopen(f,se);  % Görüntünün arka plan? elde edildi.
subplot(131), imshow(f)
...
title('Morfolojik aç?lm?? görüntü')
subplot(333), imshow(fc);
title('Morfolojik kapanm?? görüntü')
subplot(334), imshow(foc);
title({'Açmadan sonra' 'kapanm?? görüntü'})
subplot(335), imshow(foci);
title({'Kapamadan sonra' 'aç?lm?? görüntü'})
subplot(336), imshow(fasf);
title({'Açmadan sonra kapanm??' 'çe?itli yar?çaplarda görüntü'})
% Enlarge figure to full screen.
set(gcf, 'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);

%% Gri ölçekli açma ve kapama-3
...
%% Renkli görüntülerde morfolofik i?lemler
% Importing the image
I = imread('iris.tif');
subplot(2, 3, 1),
imshow(I);
